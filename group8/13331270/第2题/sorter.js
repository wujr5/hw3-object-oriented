...
function addSpanTagToEachLetter(tables) {
  for (var i = 0; i < tables.length; i++) {
    for (var j = 0; j < tables[i].tBodies[0].rows.length; j++) {
      tables[i].tBodies[0].rows[j].cells = addSpan(tables[i].tBodies[0].rows[j].cells)
    }
  }
}

function addSpan(cells) {
  for (var i = 0; i < cells.length; i++) {
    var innerHTML = "";
    for (var j = 0; j < cells[i].textContent.length; j++) {
      innerHTML += '<span>' + cells[i].textContent[j] + '</span>';
    }
